...
def hipp_rnn_adam_atLDDHH_lnglng_mlpmax_jp_cfg_func_heterogeneous(cfg):
    cfg = hipp_rnn_adam_atLDDHH_lnglng_mlpmax_jp_cfg_func(cfg)
    cfg.data['train'] = {
            'type': 'RandomClusterVectorDataset',
            }
    cfg.model['add_target_to_others'] = False
    cfg.model['add_random_target_to_others'] = False
    cfg.model['loss_type'] = 'default'
    return cfg

def hipp_rnn_adam_hopfield_heterogeneous(cfg):
    cfg = hipp_rnn_adam_atLDDHH_lnglng_mlpmax_jp_cfg_func(cfg)
    cfg.data['train'] = {
            'type': 'RandomClusterVectorDataset',
            }
    cfg.data['imgs_per_gpu'] = 128
    cfg.model = {'type': 'HippRNN',
               'seq_len': 32,
               'noise_norm': 0.07,
               'num_negs': 3,
               'hipp_head': {'type': 'HippRNNHead',
                 'rnn_kwargs': {
                 'input_size': 128,
                 'hidden_size': 256,
                 'pattern_size': 256,
...
def hipp_rnn_fastweights_heterogeneous(cfg):
    cfg = hipp_rnn_adam_atLDDHH_lnglng_mlpmax_jp_cfg_func(cfg)
    cfg.data['train'] = {
            'type': 'RandomClusterVectorDataset',
            }
    cfg.model = {'type': 'HippRNN',
               ...
               'hipp_head': {'type': 'HippRNNHead',
                ...
                'pred_mlp': {'type': 'NonLinearNeckV4',
                 'in_channels': 512,
                 'out_channels': 512,
                 'with_avg_pool': False},
                'rnn_type': 'fastweights',
                #'rnn_tile': 3},
                'rnn_tile': None},
               'randomize_noise_norm': True}
    return cfg
...
def hipp_adam_classicalhopfield_alex(cfg, hidden_size=512):
    cfg = hipp_rnn_test_cfg_func(cfg)
    cfg.model = {
            'type': 'HippRNN',
            'seq_len': 8,
...
def hipp_adam_classicalhopfield_alex_correlated_vecs(cfg, hidden_size=512):
    cfg = hipp_rnn_adam_atLDDHH_lnglng_mlpmax_jp_cfg_func(cfg)
    print("cfg model before", cfg.model)
    cfg.model['noise_norm'] = 0.07
