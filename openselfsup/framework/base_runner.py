...
class BaseRunner(metaclass=ABCMeta):
    ...
    def register_validation_hooks(self):
        ...
        for valid_name, valid_hook_params in self.validation_params.items():
            print("valid_name, valid_hook_params", valid_name, valid_hook_params)
    ...
    def load(self):
        ...
        resume_optimizer = self.load_params.get('resume_optimizer', True)
        
        if resume:
            ckpt_path = self.get_latest_checkpoint() or ckpt_path

        if ckpt_path and osp.exists(ckpt_path):
            self.resume_from_ckpt(ckpt_path, resume_optimizer = resume_optimizer)
            return
