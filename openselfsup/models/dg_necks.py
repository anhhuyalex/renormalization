...
@NECKS.register_module
class AHA_DG_Sparse_LinearMLP(nn.Module):
    def __init__(self, in_channels, out_channels, sparsity, 
                 knockout_rate, init_scale,
                 add_bias = False, **kwargs):
        super().__init__()
        ...
        self.mlp = nn.Linear(self.in_channels, self.out_channels, bias=add_bias)
...
#         pass
    ...
    def reset_parameters(self):
        """Custom initialization *does* make a big difference to orthogonality, even with inhibition
            adapted from https://github.com/Cerenaut/pt-aha/blob/7f9b6e9c5bcdeccf0ccd365d0edbaab5b432b536/cls_module/cls_module/components/dg.py
        """
        input_area = self.in_channels
        ...
        self.mlp.weight.data = initial_values
        
    def forward(self, x):
        with torch.no_grad():
            out = SparseMLP.forward(x)
        return out
