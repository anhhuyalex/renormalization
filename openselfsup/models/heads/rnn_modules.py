...
class SelfLSTM_MST(SelfLSTM):
        ...
        if self.num_layers == 1:
            ...
            for i in range(self.num_layers):
                if i==0:
                    ...
                    ih.append(nn.Linear(self.hidden_size, 4 * self.output_size))
                    hh.append(nn.Linear(self.output_size, 4 * self.output_size))
                else:
                    ih.append(nn.Linear(self.hidden_size, 4 * self.hidden_size))
                    hh.append(nn.Linear(self.hidden_size, 4 * self.hidden_size))
    ...
    def get_zero_states(self, input):
        ...
        for i in range(self.num_layers):
            if i==0:
                h_zeros.append(torch.zeros(input.size(1), self.hidden_size, dtype=input.dtype, device=input.device))
                c_zeros.append(torch.zeros(input.size(1), self.hidden_size, dtype=input.dtype, device=input.device))
                
            elif i==self.num_layers-1:
                h_zeros.append(torch.zeros(input.size(1), self.output_size, dtype=input.dtype, device=input.device))
                c_zeros.append(torch.zeros(input.size(1), self.output_size, dtype=input.dtype, device=input.device))
    ...
    def forward(self, input, hx=None):
        """
            Expect input to be [seq, batch_size, feat_dim)
        """
        if hx is None:
            hx = self.get_zero_states(input)
        num_seq = input.size(0)
        ...
        for idx in range(num_seq):
            hx = self.forward_one_time(
                    input[idx], hx)
            outputs.append(hx[0][-1]) # append hidden state, final layer
            for idx in range(len(hx)):
                all_states[idx].append(hx[idx]) # append hidden state, cell state
        outputs = torch.stack(outputs, 0)
