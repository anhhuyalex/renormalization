...
class HippRNN(nn.Module):
    ...
    def forward_train(self, embd, target):
        ...
        print("Positive, negative, kNN pred, kNN pred neg loss", losses['pred_error'], "seq_vecs", seq_vecs.shape)
...
class MSTRNN(nn.Module):
    ...
    def forward_train(self, embd, target):
        ...
        # embd is [batch_size, seq, feat_dim)
        # transposed_embd should be [seq, batch_size, feat_dim)
        # pred_embd should be [seq, batch_size, 3]
        transposed_embd = torch.transpose(embd, 0, 1)
        
        pred_embd, pat_resps = self.hipp_head(transposed_embd)
#         print("transposed_embd", transposed_embd.shape)
#         print("pred_embd", pred_embd.shape, len(pred_embd))
#         raise ValueError
        
        pred_embd = pred_embd.permute(1, 2, 0)
        seq_loss = self.loss(pred_embd, target)
        ...
        if DEBUG:
            pdb.set_trace()
        if self.loss_type == 'default':
            losses['loss'] = seq_loss
        else:
            raise NotImplementedError(f"loss_type {self.loss_type} not supported")
        print("Positive, negative, kNN pred, kNN pred neg loss", losses['loss'])
        return losses
