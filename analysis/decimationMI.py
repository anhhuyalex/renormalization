...
#             print(sample, p_xy)
        if (row > data.shape[-1] - 3) or (col > data.shape[-1] - 6):
            rectangle = np.pad(data[sample], 6, mode = "wrap")[row:(row + 3), col:(col+6)]
        else:
            rectangle = data[sample, row:(row + 3), col:(col+6)]
        # boundary_x = rectangle[1,1] # decimation (take only center)
        boundary_x = (np.sum(rectangle[:,:3]) - rectangle[1, 1]) # average without center

        p_x[get_key(boundary_x)] += 1
        # boundary_y = rectangle[1,4] # decimation (take only center)
        boundary_y = (np.sum(rectangle[:,3:]) - rectangle[1, 4])# average without center
...
def count(chunk, mode = "individual"):
    ...
    for arr in chunk:
        if mode == "individual":
            ...
            data = 2*data.astype("float64")-1
            # print(set(tuple(data.flatten())))
        elif mode == "batch":
            data = np.sign(arr).astype("float64")
        ...
        for _ in range(n_samples):
            ...
            # boundary_y = rectangle[1,4] # decimation (take only center)
            boundary_y = (np.sum(rectangle[:,3:]) - rectangle[1, 4])# average without center
            # if boundary_x == 0 or boundary_y == 0:
            #     print(rectangle)
            #     try:
            #         print("pad wrap", np.pad(data[sample], 6, mode = "wrap")[row:(row + 3), col:(col+6)])
            #         print("pad const", np.pad(data[sample], 6, mode = "constant")[row:(row + 3), col:(col+6)])
            #     except:
            #         pass
            #     print("sample", data[sample, row:(row + 3), col:(col+6)])
            p_y[get_key(boundary_y)] += 1
            joint_boundary = np.array([np.hstack((boundary_x, boundary_y))])
...
def computeMI(results):
    ...
    print("p_xy_totals values", p_xy_totals.values())
    print("sum of p_xy_totals values", np.sum(p_xy_totals.values()))
...
if __name__ == "__main__":
    ...
    if sys.argv[1] == "2187":
        ...
        for chunk in range(4):
            result_ids.append(count.remote(file_chunks[chunk], mode="individual"))
    ...
    print("MI", mi)
    with open("average_nodiscretization_mi_nocenter.txt", "a") as myfile:
