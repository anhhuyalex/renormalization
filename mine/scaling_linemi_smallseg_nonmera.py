...
class Mine(nn.Module):
    ...
    def forward(self, x, zleft, zright):
        ...
        z_marg_left = zleft[permutation]
        z_marg_right = zright[permutation]
    ...
    def mi(self, x, zleft, zright):
        ...
        if isinstance(zleft, np.ndarray):
            zleft = torch.from_numpy(zleft).float()
        if isinstance(zright, np.ndarray):
            zright = torch.from_numpy(zright).float()
...
class AltNet(nn.Module):
    ...
    """
    def __init__(self, x_dim, y_dim, H = 100, num_hidden_layers = 2):
        super(AltNet, self).__init__()
        
        self.x_fc1 = nn.Linear(x_dim + y_dim, H)
        self.num_hidden_layers = num_hidden_layers
        self.x_hid_fcs = [nn.Linear(H, H).cuda() for _ in range(self.num_hidden_layers)]
        self.x_out_fc = nn.Linear(H, 1)
        
    
    
    def forward(self, x, yleft, yright):
        xy = torch.cat([x, yleft, yright], 1) 
        
        inter_xy = F.relu(self.x_fc1(xy)) # dim [batch, 100]
        for idx, x_hid_fc in enumerate(self.x_hid_fcs):
            inter_xy = inter_xy + F.relu(x_hid_fc(inter_xy)) # dim [batch, 100]

        h2 = (self.x_out_fc(inter_xy))
        return h2    
