    ...
    def train(self):
        ...
        for self.epoch in range(num_epochs):  # loop over the dataset multiple times
            ...
            for self.iter, data in enumerate(self.trainloader, 0):
                ...
                inputs = inputs.cuda()
                labels = labels.cuda()
                X,y=inputs[:self.data_params["num_inputs"]], inputs[self.data_params["num_inputs"]:]
...
def get_polynomial_configs(args):
    ...
    if args.model_name == "ridge":
        net = lambda: Ridge(alpha=args.ridge_alpha)
    else:
    ...
    train_params = dict(batch_size = 1, 
