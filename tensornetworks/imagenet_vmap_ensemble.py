...
model_names = sorted(name for name in vision_models.__dict__
...
parser.add_argument('--num_independent_samples', default="1", type=str,
                    help='number of augmentations to show ensemble (default: 1)',
                    dest='num_independent_samples')
parser.add_argument(
            '--gaussian_blur', 
...
class TileImagenet(datasets.ImageFolder):
    def __init__(self, root = "./data",  
        ...
        self.num_independent_samples = num_independent_samples
    ...
    def __getitem__(self, index: int):
        first_augmentation, target = super(TileImagenet, self).__getitem__(index)
        samples = []
        for i in range(self.num_independent_samples):
            sample = first_augmentation.detach().clone()
            if self.do_gaussian_blur:
                sample = self.gaussianblur(sample)
            if torch.rand(1) < 0.5:
                sample = self.left_right_flip(sample)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.pretrained:
        ...
        models = [vision_models.__dict__[args.arch]() for _ in range(args.num_models_ensemble)] 
    ...
    if args.distributed:
        ...
        if torch.cuda.is_available():
            if args.gpu is not None:
                ...
                raise ValueError("Sorry not supported in this script")
    ...
    if args.resume:
        raise ValueError("Sorry, resuming is not supported in this script")
...
#             raise ValueError("=> no record found at '{}/{}'".format(args.save_dir, args.resume))
    ...
    if args.image_transform_loader == "dummy":
        ...
        if args.num_independent_samples == "num_models_ensemble":
            args.num_independent_samples = args.num_models_ensemble
        else:
            args.num_independent_samples = 1
        train_dataset = TileImagenet(
            root = traindir,  
            phase = "train", 
            tile = [1,1],
            num_independent_samples = args.num_independent_samples,
    ...
    for epoch in range(record.curr_epoch, record.train_params.num_train_epochs):
        ...
        # train for one epoch
        train_losses, train_acc5, train_acc1, train_submodel_losses, train_submodel_top1, train_submodel_top5 = train(train_loader, models, criterion, optimizers, epoch, device, args, record, record_weights)

        # evaluate on validation set
        val_losses, val_acc5, val_acc1, val_submodel_losses, val_submodel_top1, val_submodel_top5 = validate(val_loader, models, criterion, args)
...
def train(train_loader, models, criterion, optimizers, epoch, device, args, record, record_weights):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        ensemble_outputs = []
        for (m, model, optimizer) in zip(range(args.num_models_ensemble), models, optimizers):
            optimizer.zero_grad()
            
            # compute output and loss
            output = model(images[:, m % (images.shape[1]), :, :])
            loss = criterion(output, target)
            
            # measure accuracy and record loss
            acc1, acc5 = utils.accuracy(output, target, topk=(1, 5))
            submodel_losses[m].update(loss.item(), images.size(0))
        ...
        ensemble_loss = criterion(ensemble_outputs, target)
                
        # measure accuracy and record loss
        acc1, acc5 = utils.accuracy(ensemble_outputs, target, topk=(1, 5))
        losses.update(ensemble_loss.item(), images.size(0))
    ...
    def run_validate(loader, base_progress=0):
        with torch.no_grad():
            ...
            for i, (images, target) in enumerate(loader):
                ...
                ensemble_outputs = []
                for (m, model) in zip(range(args.num_models_ensemble), models):
                    # compute output and loss
                    output = model(images[:, m % (images.shape[1]), :, :])
                    loss = criterion(output, target)
